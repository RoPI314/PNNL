...
- Feedforward multi-layer networks with configurable activations (ReLU,
  Linear, Sigmoid, Tanh).
- Backpropagation for supervised training, plus a lower-level API for
  plugging in other training methods (e.g. reinforcement learning).
- Cloning and mutation support for evolutionary strategies / genetic
  algorithm style training.
- Versioned binary save/load format.
...
`load_network` returns an empty network on failure rather than crashing.
Save files are versioned, so loading a file saved by an incompatible
version fails cleanly rather than misreading data. See
`Storage Changelog.md` for the format's version history.
...
`clone_network` performs a full deep copy, so mutating a clone never affects
the original.